...
model REECB1_modified
  ...
  parameter OpenRESV.Types.Time Trv = 0 "Filter time constant for voltage measurement (0.01 - 0.02)";
  ...
  parameter OpenRESV.Types.Time Tvpc = 0 "Filter time constant for voltage measurement (VPC)";
  parameter OpenRESV.Types.PerUnit dbd1_vpc = -0.05 "Voltage error dead band lower threshold (VPC)";
  parameter OpenRESV.Types.PerUnit dbd2_vpc = 0.05 "Voltage error dead band upper threshold (VPC)";
  parameter Real Kvpc = 0 "Reactive current injection gain during over and undervoltage conditions (VPC)";
  parameter OpenRESV.Types.PerUnit Iqh_vpc = 1.05 "Upper limit on reactive current injection Iqinj (VPC))";
  parameter OpenRESV.Types.PerUnit Iql_vpc= -1.05 "Lower limit on reactive current injection Iqinj (VPC)";
  parameter OpenRESV.Types.PerUnit vref0 = 1 "User defined voltage reference (0.95 - 1.05)";
  parameter OpenRESV.Types.Time Tp = 0.05 "Filter time constant for electrical power (0.01 - 0.1)";
  parameter OpenRESV.Types.PerUnit Qmax = 0.4360 "Upper limits of the limit for reactive power regulator (0.4 - 1.0)";
  parameter OpenRESV.Types.PerUnit Qmin = -0.4360 "Lower limits of the limit for reactive power regulator (-1.0 - -0.4)";
  ...
  parameter OpenRESV.Types.PerUnit Imax = 1.82 "Maximum limit on total converter current (1.1 - 1.3)";
  parameter OpenRESV.Types.Time Tpord = 0.02 "Power filter time constant (0.01 - 0.02) ";
  ...
  OpenRESV.Electrical.Renewables.PSSE.ElectricalController.BaseClasses.CurrentLimitLogicREECB
    ccl(
    start_ii=-Iq0,
    start_ir=Ip0,
    Imax=Imax)
  ...
  Modelica.Blocks.Nonlinear.DeadZone dbd1_dbd2(uMax=dbd2, uMin=dbd1)
    annotation (Placement(transformation(extent={{-204,144},{-184,164}})));
  Modelica.Blocks.Math.Gain gain2(k=Kqv)
    annotation (Placement(transformation(extent={{-164,144},{-144,164}})));
  OpenRESV.NonElectrical.Continuous.SimpleLag simpleLag_VPC(
    K=1,
    T=Tvpc,
    y_start=V0)
    annotation (Placement(transformation(extent={{-216,230},{-196,250}})));
  Modelica.Blocks.Math.Add add_VPC(k1=-1)
    annotation (Placement(transformation(extent={{-180,224},{-160,244}})));
  Modelica.Blocks.Sources.RealExpression VREF_VPC(y=Vref0)
    annotation (Placement(transformation(extent={{-216,202},{-196,222}})));
  Modelica.Blocks.Nonlinear.DeadZone dbd_VPC(uMax=dbd2_vpc, uMin=dbd1_vpc)
    annotation (Placement(transformation(extent={{-140,224},{-120,244}})));
  Modelica.Blocks.Math.Gain gain_VPC(k=Kvpc)
    annotation (Placement(transformation(extent={{-104,224},{-84,244}})));
  Modelica.Blocks.Nonlinear.Limiter limiter_VPC(uMax=Iqh_vpc, uMin=Iql_vpc)
    annotation (Placement(transformation(extent={{-62,224},{-42,244}})));
protected
...
equation
  ...
  connect(add3_1.y, variableLimiter.u)
  ...
  connect(dbd1_dbd2.y,gain2. u)
    annotation (Line(points={{-183,154},{-166,154}}, color={0,0,127}));
  connect(gain2.y,limiter. u)
    annotation (Line(points={{-143,154},{-126,154}},
                                                 color={0,0,127}));
  connect(add.y, dbd1_dbd2.u)
    annotation (Line(points={{-225,154},{-206,154}}, color={0,0,127}));
  connect(simpleLag_VPC.y, add_VPC.u1)
    annotation (Line(points={{-195,240},{-182,240}}, color={0,0,127}));
  connect(VREF_VPC.y, add_VPC.u2) annotation (Line(points={{-195,212},
          {-188,212},{-188,228},{-182,228}},
                                  color={0,0,127}));
  connect(add_VPC.y, dbd_VPC.u)
    annotation (Line(points={{-159,234},{-142,234}}, color={0,0,127}));
  connect(simpleLag_VPC.u, Vt) annotation (Line(points={{-218,240},{-296,
          240},{-296,160},{-320,160}},
                            color={0,0,127}));
  connect(dbd_VPC.y, gain_VPC.u)
    annotation (Line(points={{-119,234},{-106,234}}, color={0,0,127}));
  connect(gain_VPC.y, limiter_VPC.u)
    annotation (Line(points={{-83,234},{-64,234}},   color={0,0,127}));
  connect(QFlag.y, add3_1.u3) annotation (Line(points={{221,14},{232,14},{232,60},
          {210,60},{210,72},{228,72}}, color={0,0,127}));
  connect(limiter.y, add3_1.u2) annotation (Line(points={{-103,154},{200,154},{200,
          80},{228,80}}, color={0,0,127}));
  connect(limiter_VPC.y, add3_1.u1) annotation (Line(points={{-41,234},
          {216,234},{216,88},{228,88}},
                              color={0,0,127}));
...
<a href=\"modelica://OpenRESV.UsersGuide.References\">[WECCPhotovoltaic]</a>.</li>
 ...
 Diagram(graphics={Rectangle(
          extent={{-240,268},{-20,200}},
          lineColor={238,46,47},
          pattern=LinePattern.Dash,
          lineThickness=0.5), Text(
          extent={{-104,212},{-26,200}},
          textColor={238,46,47},
          textString="NEW VPC CIRCUIT",
