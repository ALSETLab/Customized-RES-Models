...
block DerivativeLag "Derivative lag transfer function block"
  ...
  parameter OpenRESV.Types.Time T "Time constant";
...
protected
  parameter OpenRESV.Types.Time T_dummy=if abs(T) < Modelica.Constants.eps
       then 1000 else T "Lead time constant";
  parameter OpenRESV.Types.Time K_dummy=if abs(K) < Modelica.Constants.eps
